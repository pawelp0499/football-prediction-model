...
data_model_input <- df_corr
```

```{r}
summary(data_model_input)

write.csv(data_model_input, "./data_model_input.csv", row.names=FALSE) # Export dataframe to csv file
```
...
sample = sample.split(data_model_input,SplitRatio = 0.70) # Splitting the data 70:30 ratio into training and test set
train = subset(data_model_input,sample == TRUE) 
test = subset(data_model_input, sample == FALSE)
